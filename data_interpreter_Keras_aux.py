...
class dataInterpreter(object):
    def __init__(self, inputAtts, targetAtts=['derived_speed'], includeUser=True, includeSport=False, includeGender=False, includeTemporal=False, fn="endomondoHR_proper.json", scaleVals=True, trimmed_workout_len=450, scaleTargets="scaleVals", trainValidTestSplit=[.8,.1,.1], zMultiple=5, trainValidTestFN=None):
        self.filename = fn
        #self.data_path = "D:/College/Licenta/RecSys - Data Sets"
        self.data_path = "D:/OneDrive/Education/College/Licenta/RecSys - Data Sets"
    ...
    def dataIteratorSupervised(self, trainValidTest):
        targetAtts = self.targetAtts
        #TODO... split........... UGH
        #split for heart_rate, fara split pt speed_aux
        inputAtts = self.inputAtts.split(',')      #heart_rate_aux
        #inputAtts = self.inputAtts           #speed_aux
        ...
        for idx in indices:
            ...
            for i, att in enumerate(inputAtts):
                #for i in range(0, len(inputAtts)):
                #att = inputAtts[i]  # merge asa?
                if att == 'time_elapsed':
            ...
            if self.includeTemporal:
                ...
                context_outputs = np.zeros([targetDataDim, self.trimmed_workout_len])
                for i, att in enumerate(inputAtts):
                #TODO for i in range(0,len(inputAtts)):
                #    att = inputAtts[i]              #merge asa?
                    if att == 'time_elapsed':
    ...
    def scale_data(self, scaling=True): 
        ...
        # write to disk
        #np.save([self.original_data], self.processed_path)     #Versiunea lor
        np.save(self.processed_path,[self.original_data])       #Varianta corecta in numpy
        pass
