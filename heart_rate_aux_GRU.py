...
from keras.layers import Embedding, Input, Dense, Dropout, GRU
...
def parse():
    ...
    parser.add_argument('--temporal', default=True, action='store_true', help='use temporal input')
    parser.add_argument('--batch_size', default=128, type=int, help='batch size')  #
    parser.add_argument('--attribute_dim', default=5, type=int, help='attribute dimension')  #
    parser.add_argument('--hidden_dim', default=64, type=int, help='rnn hidden dimension')  #
    parser.add_argument('--lr', default=0.005, type=float,
                        help='learning rate')  # 0.001 for fine tune; 0.005 for general
    ...
    parser.add_argument('--output_reg', default=0.0, type=float, help='final output layer reg')
...
class keras_endoLSTM(object):
    def __init__(self, args, newModel):
        ...
        if newModel:
            self.model_save_location = "D:/OneDrive/Education/College/Licenta/RecSys - Data Sets/fitrec/model_states/"
            self.summaries_dir = "D:/OneDrive/Education/College/Licenta/RecSys - Data Sets/fitrec/logs/"
            ...
            self.endo_reader = dataInterpreter(self.inputAtts, self.targetAtts, self.includeUser, self.includeSport,
                                               self.includeGender, self.includeTemporal,
                                               fn=self.data_path, scaleVals=self.scale,
    ...
    def build_model(self, args):
        ...
        if self.includeTemporal:
            ...
            context_layer_1 = GRU(self.hidden_dim, return_sequences=True, input_shape=(self.num_steps, self.input_dim),
                                  name='context_layer_1')
            context_layer_2 = GRU(self.hidden_dim, return_sequences=True, input_shape=(self.num_steps, self.output_dim),
                                  name='context_layer_2')
        ...
        layer1 = GRU(self.hidden_dim, return_sequences=True, input_shape=(self.num_steps, self.layer1_dim),
                     name='layer1')(predict_vector)
        dropout1 = Dropout(0.2, name='dropout1')(layer1)
        layer2 = GRU(self.hidden_dim, return_sequences=True, name='layer2')(dropout1)
    ...
    def run_model(self, model):
        ...
        for iteration in range(1, self.max_epochs):
            ...
            checkpoint = ModelCheckpoint(model_save_fn, verbose=1, save_best_only=False, save_weights_only=False,
                                         mode='auto', period=1)
            early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=2, verbose=1, mode='auto')

            history = model.fit_generator(trainDataGen, train_steps_per_epoch, epochs=1, verbose=1,
                                          callbacks=[checkpoint])
            try:
                del history.model
                with open(self.summaries_dir + "model_history_" + self.model_file_name + "_epoch_" + str(iteration),
                          "wb") as f:
            ...
            try:
                with open(self.summaries_dir + "model_valid_score_" + self.model_file_name + "_epoch_" + str(iteration),
                          "wb") as f:
        ...
        """
        best_model = keras.models.load_model(
            'D:\\OneDrive\\Education\\College\\Licenta\\RecSys - Current\\FitRecStuff\\FR_LSTM_vs_GRU\\FR_GRU_Model\\model_states\\userId_sport_gender_context_2020-06-09-18-08-45\\userId_sport_gender_context_2020-06-09-18-08-45_bestValidScore'
            , custom_objects=custom_ob)
        ...
        test_pred = best_model.predict_generator(testDataGen, test_steps_per_epoch)
        print("Test score: " + str(test_score))

        a = self.endo_reader.generator_for_autotrain(1920, self.num_steps, "test")
        a = [x for x in a]
        path = 'D:\\OneDrive\\Education\\College\\Licenta\\RecSys - Current\\FitRecStuff\\FR_LSTM_vs_GRU\\grafice'
        for workout in range(0, 1921):
            plt.close(fig='all')
            plt.figure("Precizie GRU")
            plt.title('Precizie FitRec-GRU')
            plt.plot(a[0][1][workout])
            plt.plot(test_pred[workout])
            plt.legend(('Puls real', 'Predictie FitRec-GRU'))
            plt.xlabel('Timp')
            plt.ylabel('Puls')
            plt.show()
            # plt.savefig(path + "FitRecGRU_" + str(workout))
